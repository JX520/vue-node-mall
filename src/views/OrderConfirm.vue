      ...
      <div class="checkout-order">
        ...
        <div class="order-foot-wrap">
          ...
          <div class="next-btn-wrap">
            <button class="btn btn--m btn--red" @click="pay">确认付款</button>
          </div>
        </div>
      </div>
    </div>
    ...
    methods: {
      init() {
        axios.get("/users/cartList").then((response) => {
          ...
          this.allPrice = this.totalPrice + 230
        });
      },

      pay(){
        var addressId = this.$route.query.addressId;
        axios.post('/users/pay',{
          addressId:addressId,
          orderTotal:this.allPrice,
        }).then((response)=>{
          let res = response.data;
          if(res.status == '0'){
            console.log('订单创建成功！',res.order);
            this.$router.push({
              path:'/orderSuccess?orderId='+res.result.order.orderId
            })
          }
        })
      }
